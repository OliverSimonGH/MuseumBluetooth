...
public class DBSQLiteHelper extends SQLiteOpenHelper{   // Extending the Android SQLDB class that offers all the SQL executable commands
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String history = MessageFormat.format("CREATE TABLE IF NOT EXISTS {0}({1} INTEGER PRIMARY KEY AUTOINCREMENT, {2} VARCHAR, {3} INTEGER, {4} INTEGER)", HISTORY_NAME, HISTORY_ID, BEACON_UUID, BEACON_MAJORNO, BEACON_MINORNO);
        db.execSQL(history);
    ...
    public boolean insertHistoryData(String beaconUUID, int beaconMajorNo, int beaconMinorNo){
        SQLiteDatabase db = this.getWritableDatabase();
